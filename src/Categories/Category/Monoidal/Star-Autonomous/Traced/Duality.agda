...
-- The dualizing-object argument follows Hajgató--Hasegawa 2013,
-- Lemmas 3.2, 3.3, and 3.5, and Corollary 3.4.
...
open import Categories.Category.Monoidal.Closed.SelfEnrichment.Currying symmetric Cl
  using ( _⊗₁ˡ_; _⊗₁ʳ_; ev⊗ˡ; ev⊗ʳ; tensorHelper
        ; tensorˡ-action; tensorˡ-eval; tensorʳ-action; tensorʳ-eval )
...
open import Categories.Enriched.Bifunctor symmetric
  using ( BifunctorHelper; appʳ; op₂F; reduce-⊠; reduce-flipped-helper
...
open Base public
  using ( swap-curry-≅; swap-curry-natural; swap-curry⇐-commʳ
        ; δ; δ-natural; δ-name; uncurry-δ; uncurry-δ-at; IsDualizing )
...
module Dualized (⊥ : Obj) (dualizing : IsDualizing ⊥) where
  ...
  private
    ...
    abstract
      **-natural⁻¹ : {f : A ⇒ B} →
        f ∘ **-≅.to ≈ **-≅.to ∘ [ [ f , id ]₁ , id ]₁
      **-natural⁻¹ {f = f} = ⟺ (conjugate-from **-≅ **-≅ (δ-natural f))

      δ²-comm : (f : W ⇒ A) →
        [ [ id ⊗₁ f , id ]₁ , id ]₁ ∘ insert-double-dual-≅.from
        ≈ insert-double-dual-≅.from ∘ (id ⊗₁ f)
      δ²-comm f = δ-natural (id ⊗₁ f)

      swap-curry-layer-comm : (f : W ⇒ A) →
        [ [ f , id ]₁ , id ]₁ ∘ swap-curry-layer-≅.from
        ≈ swap-curry-layer-≅.from ∘ [ [ id ⊗₁ f , id ]₁ , id ]₁
      swap-curry-layer-comm f = begin
        [ [ f , id ]₁ , id ]₁ ∘ [ swap-curry-≅.to , id ]₁
          ≈˘⟨ [-,-].homomorphism ⟩
        [ swap-curry-≅.to ∘ [ f , id ]₁ , id ∘ id ]₁
          ≈⟨ [-,-].F-resp-≈ (swap-curry⇐-commʳ f , Equiv.refl) ⟩
        [ [ id ⊗₁ f , id ]₁ ∘ swap-curry-≅.to , id ∘ id ]₁
          ≈⟨ [-,-].homomorphism ⟩
        [ swap-curry-≅.to , id ]₁ ∘ [ [ id ⊗₁ f , id ]₁ , id ]₁ ∎

      replace-dual-unit-comm : (f : W ⇒ A) →
        [ [ f , id ]₁ , id ]₁ ∘ replace-dual-unit-≅.from
        ≈ replace-dual-unit-≅.from ∘ [ [ f , id ]₁ , id ]₁
      replace-dual-unit-comm f = begin
        [ [ f , id ]₁ , id ]₁ ∘ [ [ id , ⊥*-≅.to ]₁ , id ]₁
          ≈˘⟨ [-,-].homomorphism ⟩
        [ [ id , ⊥*-≅.to ]₁ ∘ [ f , id ]₁ , id ∘ id ]₁
          ≈⟨ [-,-].F-resp-≈ ([ [-,-] ]-commute , Equiv.refl) ⟩
        [ [ f , id ]₁ ∘ [ id , ⊥*-≅.to ]₁ , id ∘ id ]₁
          ≈⟨ [-,-].homomorphism ⟩
        [ [ id , ⊥*-≅.to ]₁ , id ]₁ ∘ [ [ f , id ]₁ , id ]₁ ∎
    ...
    abstract
      feedback⁻-comm : (f : W ⇒ A) →
        [ [ f , id ]₁ , id ]₁ ∘ feedback⁻.from
        ≈ feedback⁻.from ∘ (id ⊗₁ f)
      feedback⁻-comm f = begin
        [ [ f , id ]₁ , id ]₁ ∘ feedback⁻.from
          ≈⟨ extendʳ (replace-dual-unit-comm f) ⟩
        replace-dual-unit-≅.from ∘
          ([ [ f , id ]₁ , id ]₁ ∘ (swap-curry-layer-≅.from ∘ **-≅.from))
          ≈⟨ refl⟩∘⟨ extendʳ (swap-curry-layer-comm f) ⟩
        replace-dual-unit-≅.from ∘
          (swap-curry-layer-≅.from ∘ ([ [ id ⊗₁ f , id ]₁ , id ]₁ ∘ **-≅.from))
          ≈⟨ refl⟩∘⟨ refl⟩∘⟨ δ²-comm f ⟩
        replace-dual-unit-≅.from ∘
          (swap-curry-layer-≅.from ∘ (**-≅.from ∘ (id ⊗₁ f)))
          ≈⟨ assoc²εβ ⟩
        feedback⁻.from ∘ (id ⊗₁ f) ∎
      ...
      feedback⁻-comm⁻¹ : (f : W ⇒ A) →
        (id ⊗₁ f) ∘ feedback⁻.to
        ≈ feedback⁻.to ∘ [ [ f , id ]₁ , id ]₁
      feedback⁻-comm⁻¹ f = ⟺ (conjugate-from feedback⁻ feedback⁻ (feedback⁻-comm f))

      feedback⁻-hom : (f : W ⇒ A) (g : B ⇒ Z) →
        [ g , id ⊗₁ f ]₁ ∘ [ id , feedback⁻.to ]₁
        ≈ [ id , feedback⁻.to ]₁ ∘ [ g , [ [ f , id ]₁ , id ]₁ ]₁
      feedback⁻-hom f g = begin
        [ g , id ⊗₁ f ]₁ ∘ [ id , feedback⁻.to ]₁
          ≈˘⟨ [-,-].homomorphism ⟩
        [ id ∘ g , (id ⊗₁ f) ∘ feedback⁻.to ]₁
          ≈⟨ [-,-].F-resp-≈ (id-comm-sym , feedback⁻-comm⁻¹ f) ⟩
        [ g ∘ id , feedback⁻.to ∘ [ [ f , id ]₁ , id ]₁ ]₁
          ≈⟨ [-,-].homomorphism ⟩
        [ id , feedback⁻.to ]₁ ∘ [ g , [ [ f , id ]₁ , id ]₁ ]₁ ∎
    ...
    abstract
      ...
      φᵀ⇐-comm f g = begin
        [ g , id ⊗₁ f ]₁ ∘ φᵀ-≅.to
          ≈⟨ extendʳ (feedback⁻-hom f g) ⟩
        [ id , feedback⁻.to ]₁ ∘
          ([ g , [ [ f , id ]₁ , id ]₁ ]₁ ∘ curry₂)
          ≈⟨ pushʳ curry₂-natural ⟩
        φᵀ-≅.to ∘ [ g ⊗₁ [ f , id ]₁ , id ]₁ ∎

      transpose-dual-comm : (f : W ⇒ A) (g : B ⇒ Z) →
        [ [ g ⊗₁ [ f , id ]₁ , id {⊥} ]₁ , id ]₁ ∘ [ φᵀ-≅.to , id ]₁
        ≈ [ φᵀ-≅.to , id ]₁ ∘ [ [ g , id ⊗₁ f ]₁ , id {⊥} ]₁
      transpose-dual-comm f g = begin
        [ [ g ⊗₁ [ f , id ]₁ , id ]₁ , id ]₁ ∘ [ φᵀ-≅.to , id ]₁
          ≈˘⟨ [-,-].homomorphism ⟩
        [ φᵀ-≅.to ∘ [ g ⊗₁ [ f , id ]₁ , id ]₁ , id ∘ id ]₁
          ≈˘⟨ [-,-].F-resp-≈ (φᵀ⇐-comm f g , Equiv.refl) ⟩
        [ [ g , id ⊗₁ f ]₁ ∘ φᵀ-≅.to , id ∘ id ]₁
          ≈⟨ [-,-].homomorphism ⟩
        [ φᵀ-≅.to , id ]₁ ∘ [ [ g , id ⊗₁ f ]₁ , id ]₁ ∎
  ...
  private
    ...
    target₁ʳ : Target.₁ {X = X , X} {Y = X , Y}
               ≈ ⊗-map₁ ∘ ((id ⊗₁ UnitHom.₁ {X = X} {Y = X}) ∘ σ⇒)
    target₁ʳ = assoc
    ...
    target-actionʳ = (target₁ʳ ⟩∘⟨refl) ○ target-tailʳ
    ...
    target-actionʳ-eval {X = X} {Y = Y} = begin
      ...
      uncurry ((X ⊗₁ˡ Y) [ X , unit ]₀)  ≈⟨ tensorˡ-eval X Y [ X , unit ]₀ ⟩
      ev⊗ˡ X Y [ X , unit ]₀ ∎

    merge-tensor-unit : (f : A ⇒ B) →
      (id ⊗₁ f) ∘ (Self.id {A = Y} ⊗₁ id) ≈ Self.id ⊗₁ f
    merge-tensor-unit f = merge₂ˡ ○ (refl⟩⊗⟨ identityʳ)

    tensor-map-precompose :
      ⊗-map₁ {Y} {Y} {[ Y , unit ]₀} {[ X , unit ]₀} ∘
    ...
    target-actionˡ-eval : (X Y : Obj) → uncurry (TargetAction.F₁ˡ {X = X} {Y})
      ≈ ev⊗ʳ Y (Self.hom Y unit) (Self.hom X unit) ∘ (homʳ₁ X Y unit ⊗₁ id)
    target-actionˡ-eval X Y = begin
      ...
      uncurry ((Y ⊗₁ʳ [ Y , unit ]₀) [ X , unit ]₀) ∘ (homʳ₁ X Y unit ⊗₁ id)
        ≈⟨ tensorʳ-eval Y [ Y , unit ]₀ [ X , unit ]₀ ⟩∘⟨refl ⟩
      ev⊗ʳ Y (Self.hom Y unit) (Self.hom X unit) ∘ (homʳ₁ X Y unit ⊗₁ id) ∎
    ...
    t-extraordinary =
      extraordinary-resp-≈ selfEnrichment selfEnrichment φ-target unit (λ X → ⟺ ⌜⌞⌟⌝ᶜ)
        (map-extraordinary selfEnrichment selfEnrichment
          (NaturalIsomorphism.from φ-NI) (TraceExtraordinary.τ-extraordinary Cl T))
  ...
  private abstract
    extraordinaryʳ : (s : unit ⇒ X ⊗₀ [ X , unit ]₀) →
      ...
      ≈ (eval ⊗₁ id) ∘ α⇐ ∘ (id ⊗₁ s) ∘ ρ⇐
    extraordinaryʳ {X = X} {Y = Y} s = begin
      uncurry (internal-∘ ∘ (TargetAction.F₁ʳ ⊗₁ ⌜ s ⌝) ∘ ρ⇐) ∘ ρ⇐
        ≈⟨ uncurry-∘-⌜⌝ʳ ⟩∘⟨refl ⟩
      (uncurry TargetAction.F₁ʳ ∘ (id ⊗₁ s)) ∘ ρ⇐
        ≈⟨ assoc ⟩
      uncurry TargetAction.F₁ʳ ∘ ((id ⊗₁ s) ∘ ρ⇐)
        ≈⟨ pushˡ target-actionʳ-eval ⟩
      (eval ⊗₁ id) ∘ α⇐ ∘ (id ⊗₁ s) ∘ ρ⇐ ∎

    precomposition-eval : (X Y : Obj) →
      ev⊗ʳ Y (Self.hom Y unit) (Self.hom X unit) ∘ (homʳ₁ X Y unit ⊗₁ id)
      ≈ (id ⊗₁ internal-∘) ∘ (id ⊗₁ σ⇒) ∘ α⇒ ∘ (σ⇒ ⊗₁ id) ∘ α⇐
    precomposition-eval X Y = begin
      ev⊗ʳ Y (Self.hom Y unit) (Self.hom X unit) ∘ (homʳ₁ X Y unit ⊗₁ id)
        ≈˘⟨ reassoc-tail₅ ⟩
      (id ⊗₁ eval) ∘ α⇒ ∘ (σ⇒ ⊗₁ id) ∘ α⇐ ∘ (homʳ₁ X Y unit ⊗₁ id)
        ≈⟨ refl⟩∘⟨ refl⟩∘⟨ refl⟩∘⟨ α⇐-⊗id-commute ⟩
      (id ⊗₁ eval) ∘ α⇒ ∘ (σ⇒ ⊗₁ id) ∘ ((homʳ₁ X Y unit ⊗₁ id) ⊗₁ id) ∘ α⇐
        ≈⟨ refl⟩∘⟨ refl⟩∘⟨ extendʳ (parallel σ⇒-comm id-comm-sym) ⟩
      (id ⊗₁ eval) ∘ α⇒ ∘ ((id ⊗₁ homʳ₁ X Y unit) ⊗₁ id) ∘ (σ⇒ ⊗₁ id) ∘ α⇐
        ≈⟨ refl⟩∘⟨ extendʳ assoc-commute-from ⟩
      (id ⊗₁ eval) ∘ (id ⊗₁ (homʳ₁ X Y unit ⊗₁ id)) ∘ α⇒ ∘ (σ⇒ ⊗₁ id) ∘ α⇐
        ≈⟨ pullˡ merge₂ˡ ⟩
      (id ⊗₁ (eval ∘ (homʳ₁ X Y unit ⊗₁ id))) ∘ α⇒ ∘ (σ⇒ ⊗₁ id) ∘ α⇐
  ...
  private
    ...
    abstract
      ...
      feedback⁻-unit = begin
        ...
        feedback⁻.from ∘ feedback⁻.to ∘ [ λ⇐ , id ]₁
          ≈⟨ cancelˡ feedback⁻.isoʳ ⟩
        [ λ⇐ , id ]₁                                             ∎
